---
title: "Omics - Analysis of large-scale biomolecular datasets Introduction to Linux"
subtitle: |
  KIMN20 - LTH
author: 
  - "Daniel Nilsson"
...
Tab completion helps you type faster by automatically completing commands, file names, and paths.

### How to Use

- Press `Tab` once: Auto-completes if there's a unique match
- Press `Tab` twice: Shows all possible completions

### Examples

```bash
ls<Tab>        # Completes command
cd Doc<Tab>    # Completes directory name
cp file<Tab>   # Shows files starting with 'file'
```
...
- Avoids typos
- Speeds up typing
- Helps discover available files/commands
...
[Linux Lab on Canvas](https://canvas.education.lu.se/courses/37606/files/7018849?module_item_id=1652958&fd_cookie_set=1)

- **RStudio Server**: [https://130.235.8.214/rstudio](https://130.235.8.214/rstudio)

A web-based integrated development environment (IDE) for the R programming language that runs on a remote Linux server and is accessed via a web browser.

- **Git Bash:**

Git Bash is a lightweight terminal for Windows bundled with Git for Windows. It provides a Bash shell and common GNU command-line tools (ssh, grep, ls, etc.), allowing you to run typical Unix/Linux commands and shell scripts on Windows. Use it to interact with Git repositories and follow Linux command-line tutorials when you don't have a native Linux/macOS terminal.

<a href="https://about.gitlab.com/blog/git-command-line-on-windows-with-git-bash/"><img src="assets/git-bash.png" alt="Git Bash" width="100" height="auto"></a>
